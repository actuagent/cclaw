...
```bash
git clone https://github.com/actuagent/cclaw.git
cd cclaw
go mod download
```
...
```bash
go run ./cmd/cclaw onboard
```

会创建 `~/.cclaw/` 下的配置与运行目录（config、sessions、memory、cron、prompts、skills 等）。

### 4) 配置模型

编辑 `~/.cclaw/config.yaml`，至少配置一个 provider 的 `apiKey` 和你要使用的模型名。

### 5) 运行

```bash
# 交互式 CLI
go run ./cmd/cclaw agent

# 单轮消息，直接在终端与 Agent 对话，支持历史记录、Markdown 渲染
go run ./cmd/cclaw agent -m "hello"

# 启动网关（飞书 + agent + heartbeat + cron）
go run ./cmd/cclaw gateway
```
...
| 命令 | 说明 |
|---|---|
| `go run ./cmd/cclaw onboard` | 初始化配置与运行目录 |
| `go run ./cmd/cclaw agent` | 交互式聊天 |
| `go run ./cmd/cclaw agent -m "..."` | 单轮聊天 |
| `go run ./cmd/cclaw agent --raw` | 不做 Markdown 渲染，直接输出原文 |
| `go run ./cmd/cclaw gateway` | 启动网关（飞书 + 心跳 + 定时任务） |
| `go run ./cmd/cclaw status` | 输出当前生效配置与路径信息 |
| `go run ./cmd/cclaw version` | 查看版本 |
...
默认配置文件：`~/.cclaw/config.yaml`（也支持 JSON / TOML）。
...
gateway:
  heartbeat:
    ...
    interval: "30m"
  cron:
    storePath: "~/.cclaw/cron/jobs.json"
```
...
- [nanobot-eino](https://github.com/iamclancyliang/nanobot-eino)（本项目复刻来源，快照与授权说明见 [UPSTREAM.md](./UPSTREAM.md)）
