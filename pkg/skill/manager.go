...
// NanobotMeta 描述 Skill 元数据中的运行条件与安装信息。
//
// 该名称为兼容上游公开类型而保留；CCLAW 原生使用 `cclaw` 元数据键，
// 同时兼容 `nanobot` 与 `openclaw` 键。
...
// 读取 CCLAW 元数据，并按顺序兼容上游生态使用的旧元数据键。
func (m *Manager) getNanobotMeta(sk *Skill) NanobotMeta {
	...
	for _, key := range []string{"cclaw", "nanobot", "openclaw"} {
