...
metadata: {"cclaw":{"emoji":"🐙","requires":{"bins":["gh"]}}}
---
...
metadata: {"cclaw":{"requires":{"env":["CCLAW_TEST_SKILL_SECRET_12345"]}}}
---

# Needs Env Skill

Requires CCLAW_TEST_SKILL_SECRET_12345.
`

const alwaysInMetaSkillMD = `---
name: always-meta
description: Always-on via cclaw metadata.
metadata: {"cclaw":{"always":true,"emoji":"🔁"}}
---
...
`)
	...
	if !strings.Contains(meta.Metadata, "cclaw") {
...
// --- 测试：通过 cclaw 元数据启用常驻 Skill ---

// 验证 cclaw 元数据键能够启用常驻 Skill。
func TestGetAlwaysSkillsCCLAWMetadata(t *testing.T) {
	dir := t.TempDir()
	builtinDir := filepath.Join(dir, "builtin")
	wsDir := filepath.Join(dir, "workspace")
	os.MkdirAll(filepath.Join(wsDir, "skills"), 0755)
...
// 验证旧 nanobot 元数据键仍保持兼容。
func TestGetAlwaysSkillsLegacyNanobotMetadata(t *testing.T) {
	dir := t.TempDir()
	builtinDir := filepath.Join(dir, "builtin")
	wsDir := filepath.Join(dir, "workspace")
	legacySkill := `---
name: legacy-always
description: Always-on via legacy metadata.
...
`
	...
	createSkillFile(t, builtinDir, "legacy-always", legacySkill)
	...
	always := m.GetAlwaysSkills()
	if len(always) != 1 || always[0] != "legacy-always" {
		t.Fatalf("expected legacy metadata to remain compatible, got %v", always)
	}
}
...
name: posix-tool
description: Uses ls.
metadata: {"cclaw":{"requires":{"bins":["ls"]}}}
---
# POSIX Tool
`
...
metadata: {"cclaw":{"requires":{"bins":["__nanobot_test_nonexistent_bin_xyz__"]}}}
---
# Missing Tool
`
...
func TestIsAvailableWithEnv(t *testing.T) {
	...
	t.Setenv("CCLAW_TEST_SKILL_SECRET_12345", "test-value")
	if !m.isAvailable(sk) {
...
metadata: {"cclaw":{"requires":{"bins":["__nanobot_test_missing_bin__"],"env":["__CCLAW_TEST_MISSING_ENV__"]}}}
---
# Multi Req
`
	...
	if !strings.Contains(msg, "ENV: __CCLAW_TEST_MISSING_ENV__") {
...
metadata: {"cclaw":{"requires":{"bins":["__nanobot_test_fake_bin__"]}}}
---
# Unavailable
`
...
metadata: {"cclaw":{"requires":{"bins":["__nanobot_test_fake_bin__"]},"install":[{"id":"brew","kind":"brew","formula":"steipete/tap/fake-tool","bins":["__nanobot_test_fake_bin__"],"label":"Install fake-tool (brew)"},{"id":"apt","kind":"apt","package":"fake-tool","label":"Install fake-tool (apt)"}]}}
---
# Needs Install
`
