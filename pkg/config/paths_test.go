...
func TestSetGetConfigPath(t *testing.T) {
	...
	custom := "/tmp/test-cclaw/config.yaml"
...
func TestGetConfigPathDefault(t *testing.T) {
	...
	if filepath.Base(filepath.Dir(got)) != ".cclaw" {
		t.Errorf("GetConfigPath() dir = %q, want parent .cclaw", filepath.Dir(got))
	}
}
...
func TestGetDataDir(t *testing.T) {
	...
	SetConfigPath("/tmp/test-cclaw/config.yaml")
	got := GetDataDir()
	if got != "/tmp/test-cclaw" {
		t.Errorf("GetDataDir() = %q, want %q", got, "/tmp/test-cclaw")
	}
}
