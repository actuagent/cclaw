...
// DefaultConfigDir 返回当前用户的 CCLAW 默认配置目录 `~/.cclaw`。
func DefaultConfigDir() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ".cclaw"
	}
	return filepath.Join(home, ".cclaw")
}

// DefaultConfigPath 返回 CCLAW 默认配置文件路径 `~/.cclaw/config.yaml`。
func DefaultConfigPath() string {
	return filepath.Join(DefaultConfigDir(), "config.yaml")
}

// DefaultConfig 返回默认配置，所有运行目录均位于 `~/.cclaw` 下。
...
// Load 从 YAML、JSON 或 TOML 文件加载配置，并应用环境变量覆盖。
// path 为空时依次在 `~/.cclaw` 与当前目录查找配置文件；配置文件不存在时返回默认配置，
// 文件读取失败或内容无法解析时返回明确错误。
...
func bindEnvVars(v *viper.Viper) {
	...
	_ = v.BindEnv("agent.provider", "CCLAW_PROVIDER")
	_ = v.BindEnv("agent.model", "CCLAW_AGENT_MODEL")
	_ = v.BindEnv("agent.maxTokens", "CCLAW_MAX_TOKENS")
	_ = v.BindEnv("agent.temperature", "CCLAW_TEMPERATURE")
	_ = v.BindEnv("agent.reasoningEffort", "CCLAW_REASONING_EFFORT")

	_ = v.BindEnv("tools.workspace", "CCLAW_WORKSPACE")
}
